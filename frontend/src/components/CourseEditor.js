  ...
  const validate = () => {
    ...
    for (let i = 0; i < sections.length; i++) {
      ...
      for (let j = 0; j < s.lessons.length; j++) {
        const L = s.lessons[j];
        if (!L.title.trim()) return `Section ${i + 1}, Lesson ${j + 1}: title required.`;
        if (!["video", "article", "quiz"].includes(L.type)) return `Section ${i + 1}, Lesson ${j + 1}: invalid type.`;
      }
    }
    return null;
  };
      ...
      const payload = {
        ...
        sections: sections.map(s => ({
          ...
          lessons: s.lessons.map(L => ({
            title: L.title.trim(),
            type: L.type,
            durationMin: L.durationMin === "" ? null : Number(L.durationMin),
            contentUrl: L.contentUrl.trim(),
          })),
        })),
      };

      const url = isEdit ? toAbsolute(`/api/courses/${id}`) : toAbsolute(`/api/courses`);
    ...
    return (
      <div className="course-page">
        ...
        <div className="course-container">
          <div className="empty">You do not have permission to access this page.</div>
        </div>
        <SiteFooter />
      </div>
    );
          ...
          <>
            ...
            <section className="card">
              ...
              <div className="form-grid">
                ...
                <div className="form-row full">
                  ...
                  />
                  {coverUrl?.trim() && <img src={coverUrl} alt="cover" className="cover-preview" />}
                </div>
              </div>
            </section>
            ...
            <section className="card">
                  ...
                  <div key={si} className="sec-card">
                    <div className="sec-head">
                      <input
                        className="sec-title"
                        value={sec.title}
                        onChange={(e) => changeSectionTitle(si, e.target.value)}
                      />
                      <div className="sec-actions">
                        <button className="mini" onClick={() => addLesson(si)}>+ Lesson</button>
                        <button className="mini danger" onClick={() => removeSection(si)}>Delete Section</button>
                      </div>
                    </div>
                      ...
                      {sec.lessons.map((ls, li) => (
                        <div key={li} className="lesson-row">
                          ...
                          <select
                            className="ls-type"
                            value={ls.type}
                            onChange={(e) => setLesson(si, li, { type: e.target.value })}
                          >
              ...
              <div className="actions">
                <button className="ghost-btn" onClick={addSection}>+ Add Section</button>
                <button className="primary-btn" disabled={saving} onClick={handleSubmit}>
                  {saving ? (isEdit ? "Updating…" : "Publishing…") : (isEdit ? "Update" : "Publish course")}
                </button>
              </div>
            </section>
          </>
